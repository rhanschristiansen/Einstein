...
class MultipleObjectTracker(object):
    def __init__(self):
        self.tracks = []
        self.face_rec_engine = None

    def _save_tracks_to_json(self):
        for track in self.tracks:
            if track.is_dead():
                np.save('tracks/' + str(track.uid), track.history)

    def _delete_duplicate_tracks(self):
    ...
    def _assign_detections_to_tracks_munkres(self, detections, frame_id, save=False):
            ...
            for det in detections:
                ...
                # established tracks search in predicted location
                    ...
                    if dist < const.MAX_PIXELS_DIST_TRACK and iou > const.MIN_IOU_TRACK:
                        ...
                        sim = self.face_rec_engine.compute_similarity(det.fvec, track.get_latest_fvec())
    ...
    def update_tracks(self, detections, frame_id, save=False):
        self._assign_detections_to_tracks_munkres(detections, frame_id)
    ...
    def get_track_heads(self):
        # TODO: return "latest" bounding boxes from tracks
        heads = self.tracks
        return heads
